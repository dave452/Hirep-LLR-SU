...
void project_to_spNg(suNg *u)
{
  int i;
  suNg_vector *v1, *v2, *v3, *v4;
  ...
  for(i=0;i<2;++i) {
    _complex_star_assign((*v3).c[i+2]);
    _complex_add_assign((*v1).c[i],(*v3).c[i+2]);
    _complex_mulr((*v1).c[i],0.5,(*v1).c[i]);

    _complex_star((*v3).c[i+2],(*v1).c[i]);
  }

  for(i=0;i<2;++i) {
    _complex_star_assign((*v4).c[i+2]);
    _complex_add_assign((*v2).c[i],(*v4).c[i+2]);
    _complex_mulr((*v2).c[i],0.5,(*v2).c[i]);
 
    _complex_star((*v4).c[i+2],(*v2).c[i]);
  }

  for(i=0;i<2;++i) {
    _complex_star_minus_assign((*v3).c[i]);
    _complex_add_assign((*v1).c[i+2],(*v3).c[i]);
    _complex_mulr((*v1).c[i+2],0.5,(*v1).c[i+2]);
 
    _complex_star((*v3).c[i],(*v1).c[i+2]);
    _complex_minus((*v3).c[i],(*v3).c[i]);
  }

  for(i=0;i<2;++i) {
    _complex_star_minus_assign((*v4).c[i]);
    _complex_add_assign((*v2).c[i+2],(*v4).c[i]);
    _complex_mulr((*v2).c[i+2],0.5,(*v2).c[i+2]);
 
    _complex_star((*v4).c[i],(*v2).c[i+2]);
    _complex_minus((*v4).c[i],(*v4).c[i]);
  }
